...
static GdkPixbuf *
xfce_thumbnail_load (const gchar *thumbnail_path,
                     const gchar *uri,
                     time_t mtime,
                     GError **error)
{
...
static gboolean
xfce_thumbnail_save (GdkPixbuf *thumbnail,
...
/**
 * xfce_thumbnail_get_for_file:
 * @filename : the absolute path to the file for which to load or generate a thumbnail.
 * @size     : the desired thumbnail size, either %XFCE_THUMBNAIL_SIZE_NORMAL or %XFCE_THUMBNAIL_SIZE_LARGE.
                             ...
                             GError **error)
{
  ...
  g_return_val_if_fail (size == XFCE_THUMBNAIL_SIZE_NORMAL || size == XFCE_THUMBNAIL_SIZE_LARGE, NULL);
      ...
      if (G_LIKELY (uri != NULL))
        {
          ...
          path = g_build_path ("/", g_get_user_cache_dir (), "thumbnails",
                               (size == XFCE_THUMBNAIL_SIZE_NORMAL) ? "normal" : "large",
                               name, NULL);
          g_free (name);

          /* try to load the thumbnail */
          thumbnail = xfce_thumbnail_load (path, uri, statb.st_mtime, NULL);
          if (G_UNLIKELY (thumbnail == NULL))
            {
              /* try to generate a thumbnail for the file using the available GdkPixbufLoaders */
              thumbnail = xfce_gdk_pixbuf_new_from_file_at_max_size (filename, size, size, TRUE, error);
              if (G_LIKELY (thumbnail != NULL))
                {
                  /* save the generated thumbnail into the thumbnail database */
                  if (!xfce_thumbnail_save (thumbnail, path, uri, statb.st_mtime, &err))
                    {
                      /* better let the user know whats going on, but no need to fail here */
                      g_warning ("Failed to save generated thumbnail for \"%s\" to \"%s\": %s",
                                 filename, path, err->message);
                      g_error_free (err);
                    }
                }
            }
...
/**
 * xfce_thumbnail_get_for_uri:
 * @uri   : the URI for which to load the thumbnail.
 * @size  : the desired thumbnail size.
 * @error : return location for errors or %NULL.
 *
 * Similar to xfce_thumbnail_get_for_file(), but does not try to generate
                            ...
                            GError **error)
{
  ...
  /* try to load the thumbnail */
  thumbnail = xfce_thumbnail_load (path, uri, (time_t) -1, error);
  g_free (path);
