  ...
  if (data->editor->exclusive_groups != NULL)
    {
      ...
      info.paths_in_same_groups = g_hash_table_new (g_str_hash, g_str_equal);

      // Insert all paths we know about into all_managed_paths.  Insert paths
      // that can conflict with this accelerator into paths_in_same_groups.
          ...
          for (size_t entry_idx = 0; entry_idx < section->size; ++entry_idx)
            {
              ...
              if (g_hash_table_contains (sections_in_same_groups, GUINT_TO_POINTER (array_idx)))
                {
                  g_hash_table_add (info.paths_in_same_groups, accel_path);
                }
            }
        }
